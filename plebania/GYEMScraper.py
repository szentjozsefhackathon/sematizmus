...
def GYEM(filename=None, year=None):
    ...
    for plebania_talalat in tqdm(json_content["items"]):
        ...
        plebanialista.append({
            ...
            "phones": [f"0036{telefon.strip() if not telefon.strip().startswith('06') else telefon[2:].strip()}" for telefon in plebania['phone'].strip().replace('/', '').replace('-','').replace("+36", "").replace(",",";").split(";")] if plebania["phone"] != None and plebania["phone"] != "" else [],
